 ...
 * @type String
 **/
elFinder.prototype.version = '2.1.39';
...
$.fn.elfindercwd = function(fm, options) {
        ...
        // Events for hidden input @HOBO
        .on("click selectfile  unselectall mouseenter mouseleave", function(){
						let files = []
						files = fm.selectedFiles()
            if(files.length){
              let paths = files.map(item => fm.path(item.hash))
               paths = paths.map(path => path.replace(/\\/g,"/"))
              $("#path-text-location").val(paths)
              $("#path-location").val(paths)
              $("#path-location").attr("data-small-image", `public/files/.tmb/${files[0].tmb}`)
            } else{
              $("#path-text-location").val("")
              $("#path-location").val("")
