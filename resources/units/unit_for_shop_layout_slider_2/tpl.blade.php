@if(isset($settings["images"]) && is_array($settings["images"]))
    <div id="{{isset($settings["type"]) ? $settings['type'] : 'nbullet-pagination'}}" class="gallery {{(isset($settings["type"]) && $settings['type'] == 'show-three-slides') ? 'custum_image_size' : 'custum_image_size_full' }}">
