<div class="bty-user-card-1">
    ...
    <div>
        @if(issetReturn($settings,'main_function') =='specificPost')
            @if(isset($settings['specificUser']) && \Btybug\User\User::find($settings['specificUser']))
                ...
                @if(isset($settings['name_type']) && $settings['name_type']=='dynamic')
                    <h3>@if(isset($settings['specificUser'])) {!! BBfindTableColumnData('posts',issetReturn($settings,'dynamic_name'),issetReturn($settings,'specificUser')) !!}@endif</h3>
                @endif
            @endif
        @endif
        <h4>profession</h4>
