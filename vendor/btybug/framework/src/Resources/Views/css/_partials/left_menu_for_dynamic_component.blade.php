<div class="top_left">
    ...
    <a href="{{route("create_folder_component_dynamic")}}" class="btn btn-sm btn-success pull-right custom_create_group"><i class="fa fa-plus"></i></a>
</div>
                    ...
                    <div class="panel panel-default class_for_remove">
                        <div class="panel-heading">
                            ...
                            <span class="pull-right">
                                ...
                                <a href="{{route("create_file_component_dynamic",$directory["dirname"])}}" class="btn btn-sm btn-success custom_create_new_file"><i class="fa fa-plus"></i></a>
                            </span>
                            <div class="clearfix"></div>
                        </div>
                                ...
                                @if(count($directory["children"]))
                                    @foreach($directory["children"] as $sub_group)
                                        ...
                                        <li class="custom_padding_left_0">
                                            <a href="{{route("get_content_component_dynamic")}}?type={{$original_name}}" rel="tab" class="tpl-left-items"> {{\App\Http\Controllers\PhpJsonParser::renderName(explode("_",explode('.',$sub_group->getFilename())[0]))}}</a>
    ...
    <div class="panel panel-default class_for_remove">
        <div class="panel-heading">
            ...
            <span class="pull-right">
                ...
                <a href="{{route("create_file_component_dynamic",'repl')}}" class="btn btn-sm btn-success custom_create_new_file"><i class="fa fa-plus"></i></a>
            </span>
            <div class="clearfix"></div>
        </div>
...
<script type="template" id="append_new_file">
    <li class="custom_padding_left_0">
        <a href="{{route("get_content_component_dynamic")}}?type={original_name}" rel="tab" class="tpl-left-items">{filename}</a>
        {{--<span class="inline-block pull-right">
            <button class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></button>
...
<script>
    ...
    $("body").delegate(".remove_group","click",function(){
        ...
        var that = $(this);
        var _token = $('input[name=_token]').val();
        var url = base_path + "/admin/framework/dynamic-component/removedir";
        $.ajax({
            url: url,
            data: {
                dirname:dirname,
    ...
    $("body").delegate(".go_to_save","click",function(){
        ...
        var that = $(this);
        var _token = $('input[name=_token]').val();
        var url = base_path + "/admin/framework/dynamic-component/renamefolder";
        $.ajax({
            url: url,
            data: {
                new_name:new_name,
            ...
            success: function (data) {
                if(!data.error){
                    ...
                    return (that.prev()
                        ...
                        .next()
                        .attr("href",base_path + "/admin/framework/dynamic-component/createfile/" + new_name));
                }
            },
            type: 'POST'
        });
